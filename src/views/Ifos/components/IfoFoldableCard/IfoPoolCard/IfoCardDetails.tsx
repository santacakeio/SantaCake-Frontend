...
import { Text, Flex, Box, Skeleton, TooltipText, useTooltip } from '@santacakeio/uikit'
...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const label = isCurrencyCake ? t('Max. XCake entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('XCake to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
