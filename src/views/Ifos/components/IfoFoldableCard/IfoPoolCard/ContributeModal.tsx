...
const ContributeModal: React.FC<Props> = ({
  ...
  const label = currency === tokens.cake ? t('Max. XCake entry') : t('Max. token entry')
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    poolId === PoolIds.poolBasic
      ? t(
          'For the basic sale, Max XCake entry is capped by minimum between your average XCake balance in the IFO XCake pool, or the pool’s hard cap. To increase the max entry, Stake more XCake into the IFO XCake pool',
        )
      : t(
          'For the unlimited sale, Max XCake entry is capped by your average XCake balance in the IFO XCake pool. To increase the max entry, Stake more XCake into the IFO XCake pool',
        ),
    {},
  )
      ...
      <ModalBody maxWidth="360px">
        <Box p="2px">
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              'If you don’t commit enough XCake, you may not receive any IFO tokens at all and will only receive a full refund of your XCake.',
            )}
            <Link
              fontSize="12px"
              display="inline"
              href="https://docs.santacake.io/products/ifo-initial-farm-offering"
              external
            >
              {t('Read more')}
            </Link>
