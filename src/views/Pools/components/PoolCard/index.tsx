...
import { CardBody, Flex, Text, CardRibbon } from '@santacakeio/uikit'
...
const PoolCard: React.FC<{ pool: DeserializedPool; account: string }> = ({ pool, account }) => {
  ...
  const isCakePool = earningToken.symbol === 'XCake' && stakingToken.symbol === 'XCake'
    ...
    >
      <PoolCardHeader isStaking={accountHasStakedBalance} isFinished={isFinished && sousId !== 0}>
        <PoolCardHeaderTitle
          ...
          subTitle={isCakePool ? t('Earn XCake, stake XCake') : t('Stake %symbol%', { symbol: stakingToken.symbol })}
        />
